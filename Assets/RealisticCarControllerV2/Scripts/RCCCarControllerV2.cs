...
public class RCCCarControllerV2 : MonoBehaviour {
	...
	public float engineTorque = 10000f;		// Default Engine Torque.
	public float maxEngineRPM = 10000f;		// Maximum Engine RPM.
	public float minEngineRPM = 1000f;		// Minimum Engine RPM.
	
	public float steerAngle = 90f;		// Maximum Steer Angle Of Your Vehicle.
	public float highspeedsteerAngle = 80f;		// Maximum Steer Angle At Highest Speed.
	public float highspeedsteerAngleAtspeed = 60f;		// Highest Speed For Maximum Steer Angle.
	...
	public float brake = 10000f;		// Maximum Brake Torque.
	public float maxspeed = 10000f;		//Maximum Speed.
	...
	[Range(0f, 1f)]public float maxSkidSoundVolume = 0f;
	...
	public void SmokeInit (){
		...
		if(wheelSlipAsphalt){
			...
			for(int i = 0; i < allWheelColliders.Length; i++){
                ...
                ParticleSystem psps = ps.GetComponent<ParticleSystem>();
                var em = psps.emission;
				em.enabled = false;
	...
	void CollisionParticles(Vector3 contactPoint){
		...
		for(int i = 0; i < contactSparkeList.Count; i++){
            ...
            ParticleSystem contactSparkeListps = contactSparkeList[i].GetComponent<ParticleSystem>();
            var em = contactSparkeListps.emission;
            em.enabled = true;
	...
	void Update (){
        ...
        //. Debug.Log("downForce" + downForce); brake
	...
	public void Engine (){
		...
		//Speed.
		speed = rigid.velocity.magnitude * 24f;
	...
	public void SmokeWeedEveryday (GroundMaterial ground){
		...
		for(int i = 0; i < allWheelColliders.Length; i++){
            ...
            ParticleSystem _wheelSlipAsphaltps = _wheelSlipAsphalt[i].GetComponent<ParticleSystem>();
            var em = _wheelSlipAsphaltps.emission;
		...
		if(exhaustGas.Length > 0 && engineRunning){
			...
			for(int i = 0; i < exhaustGas.Length; i++){
                ParticleSystem exhaustGasps = exhaustGas[i].GetComponent<ParticleSystem>();
                var exem = exhaustGasps.emission;
                var exmain = exhaustGasps.main;
				if(speed < 30){
                    if (!exem.enabled)
                        exem.enabled = true;
					if(gasInput > .05f){
						exem.rateOverDistance = 25;
                        exmain.startSpeed = 5;
						exmain.startSize = 5;
					}else{
                        exem.rateOverDistance = 5;
                        exmain.startSpeed = 1;
                        exmain.startSize = 3;
					}
				}else{
