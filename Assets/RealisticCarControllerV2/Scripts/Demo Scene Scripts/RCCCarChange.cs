...
public class RCCCarChange : MonoBehaviour {
	...
	void Update () {
		...
		if(selectScreen){
			mainCamera.transform.position = Vector3.Lerp(mainCamera.transform.position, objects[activeObjectIdx].transform.position + (-mainCamera.transform.forward * 12f) + new Vector3(0f, .5f, 0f), Time.deltaTime * 5f);
	...
	void OnGUI()
	{
		...
		if(selectScreen){
			...
			// Next
			if( GUI.Button(new Rect(Screen.width/2 + 65, 10, 100, 30), "下一辆") )
			{
				activeObjectIdx++;
			...
			// Previous
            if (GUI.Button(new Rect(Screen.width / 2 - 185, 10, 100, 30), "上一辆"))
			{
				activeObjectIdx--;
			...
			// Select Car
            if (GUI.Button(new Rect(Screen.width / 2 - 60, 10, 100, 30), "选择"))
			{
				selectScreen = false;
...
//				}
            ...
            if (GUI.Button(new Rect(Screen.width / 2 - 60, 10, 120, 30), "重新选择"))
			{
				selectScreen = true;
